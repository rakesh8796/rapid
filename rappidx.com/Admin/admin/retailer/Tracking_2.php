...
echo $livequery1 = "SELECT * FROM `spark_single_order` WHERE `Awb_Number`!='' AND `orderno`!='' AND `awb_gen_by`!=''  AND (`User_Id`!='' AND `User_Id`!=0) AND (`order_cancel`!='1' OR `order_cancel` IS NULL) AND `order_status_show`!='Upload' AND (`order_status_show`='Data Received' OR `order_status_show`='Not Picked')";
...
if(mysqli_num_rows($queryr)){
  // echo " : ";
  echo $couriername = $res['awb_gen_by'];
  // echo " : ";
  echo $courier_uid = $res['Single_Order_Id'];
  // echo " : ";
  echo $uploaddate = $res['Rec_Time_Date'];
  // echo " : ";


  $tdate = date('Y-m-d');
  $diff = strtotime($tdate) - strtotime($uploaddate);
  // echo " : ";
  $noofdaysgap = abs(round($diff / 86400));
  if($noofdaysgap>3){
...
echo "<br>";
$foril=1;
foreach($allawbno123 as $airwaybillnumber){
  echo $foril.".".$airwaybillnumber."&ensp;&ensp;";
// Track Multiple Orders
...
$query123 = "INSERT INTO `testing_cronjob`(`name`, `crtdate`, `count`) VALUES ('Admin',now(),'2')";
