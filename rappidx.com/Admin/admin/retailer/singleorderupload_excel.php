...
<?php
    ...
    // while($row = mysqli_fetch_assoc($singleproqueryr)){
      ...
      $singleproquery = "SELECT * FROM `spark_single_order` WHERE `User_Id` IN ($user_ids) AND `uploadtype`='Single' AND `Awb_Number`!='' AND `order_cancel` IS NULL AND `awb_gen_by`!='' AND `Rec_Time_Date`='$tdate' ORDER BY `Single_Order_Id` DESC";
      //
