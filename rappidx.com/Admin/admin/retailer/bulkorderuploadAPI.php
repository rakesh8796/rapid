...
// Selected Data In Prcessing Orders
  ...
  $value[0] = $orderdetailsres['Name'];             // 0 Customer Name
  $value[1] = $orderdetailsres['Address'];          // 1 Address
  $value[2] = $orderdetailsres['Mobile'];           // 2  Mobile
  $value[3] = $orderdetailsres['Pincode'];          // 3  Pincode
  $value[4] = $orderdetailsres['orderno'];          // 4 Order ID
  $value[5] = $orderdetailsres['Item_Name'];        // 5 Item
  $value[6] = $orderdetailsres['Quantity'];         // 6 Quantity
  $value[7] = $orderdetailsres['Width'];            // 7 Width
  $value[8] = $orderdetailsres['Height'];           // 8 Height
  $value[9] = $orderdetailsres['Length'];           // 9 Length
  $value[10] = $orderdetailsres['Invoice_Value'];   // 10 Invoice Value
  $value[11] = $orderdetailsres['Cod_Amount'];      // 11 COD Amount if not Enter 0
  $value[12] = $orderdetailsres['State'];           // 12 State
  $value[13] = $orderdetailsres['City'];            // 13 City
  $value[14] = $orderdetailsres['additionaltype'];  // 14 AdditionalDetails(dangures Goods,Extra Care)
  $value[15] = $orderdetailsres['Actual_Weight'];   // 15 Weight
  $value[16] = $orderdetailsres['Total_Amount'];    // 16 Total Amount
  $value[17] = $orderdetailsres['pickup_id'];       // 17 pickup_id
  $value[18] = $orderdetailsres['pickup_name'];     // 18 pickup_name
  $value[19] = $orderdetailsres['pickup_mobile'];   // 19 pickup_mobile
  $value[20] = $orderdetailsres['pickup_pincode'];  // 20 pickup_pincode
  $value[21] = $orderdetailsres['pickup_gstin'];    // 21 pickup_gstin
  $value[22] = $orderdetailsres['pickup_address'];  // 22 pickup_address
  $value[23] = $orderdetailsres['pickup_state'];    // 23 pickup_state
  $value[24] = $orderdetailsres['pickup_city'];     // 24 pickup_city
  $value[25] = $orderdetailsres['Order_Type'];      // 24 Order_Type
...
if($value[25]=="Prepaid"){
    ...
    $ekrtawbno = "RPXP".$newekarttkncod;
    $ekrtawbuptate = "UPDATE `api_tokens` SET `last_ppd_awb`='$newekarttkncod',`tkn_update`=now() WHERE `couriername`='Ekart'";
    mysqli_query($conn,$ekrtawbuptate);
}else{
    $newekarttkncod = $ekarttkncod+1;
    $ekrtawbno = "RPXC".$newekarttkncod;
    $ekrtawbuptate = "UPDATE `api_tokens` SET `last_cod_awb`='$newekarttkncod',`tkn_update`=now() WHERE `couriername`='Ekart'";
    mysqli_query($conn,$ekrtawbuptate);
}
...
if($response['unauthorised']){
    ...
    echo "<br> :- ";
    echo $response['response'][0]['status'];
        // REQUEST_REJECTED
    if($response['response'][0]['status_code']){
        echo "<br> :- ";
        echo $response['response'][0]['status_code'];
    }else{
        echo "<br> :- ";
        echo $response['response'][0]['message'][0];
    }
    echo "<br> :- ";
