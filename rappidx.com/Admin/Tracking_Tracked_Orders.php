...
$datey = substr($StatusDateTime,0,4);
$timehms = substr($StatusDateTime,11,8);
$lastupdatedatetime = $datey."-".$datem."-".$dated."&ensp;".$timehms;

$StatusDateTime1 = $ekrtawbstat[$airwaybillnumber]['history'][0]['updated_datetime'];
$dated1 = substr($StatusDateTime1,8,2);
$datem1 = substr($StatusDateTime1,5,2);
$datey1 = substr($StatusDateTime1,0,4);
$timehms1 = substr($StatusDateTime1,11,8);
$lastupdatedate = $datey1."-".$datem1."-".$dated1."&ensp;".$timehms1;


$StatusDateTime2 = $ekrtawbstat[$airwaybillnumber]['history'][0]['updated_datetime'];
$dated2 = substr($StatusDateTime2,8,2);
$datem2 = substr($StatusDateTime2,5,2);
$datey2 = substr($StatusDateTime2,0,4);
$timehms2 = substr($StatusDateTime2,11,8);
$pickupdate = $datey2."-".$datem2."-".$dated2."&ensp;".$timehms2;
$pickupdatetime = $pickupdate;

$ekartStatusCode = $ekrtawbstat[$airwaybillnumber]['history'][0]['status'];
$ekartStatusCodeshow = $ekrtawbstat[$airwaybillnumber]['history'][0]['public_description'];


if($ekartStatusCode=="delivered"){
    $delivereydate = $lastupdatedate;
    $delivereydatetime = $lastupdatedate;    
}elseif($ekartStatusCode=="rto"){
    $rtodate = $lastupdatedate;
    $rtodatetime = $lastupdatedate;    
}

echo $ekartstatus = "UPDATE `spark_single_order` SET
`Last_Time_Stamp`='$lastupdatedatetime',`Last_Stamp_Date`='$lastupdatedate',`pickupdate`='$pickupdate',
`pickupdatetime`='$pickupdatetime',`delivereddate`='$delivereydate',`delivereddatetime`='$delivereydatetime',
`rtodate`='$rtodate',`rtodatetime`='$rtodatetime',
`order_status`='$ekartStatusCode',`order_status1`='$ekartStatusCodeshow',`order_status_show`='$ekartStatusCode'
 WHERE `Single_Order_Id`='$courier_uid'";
mysqli_query($conn,$ekartstatus);
