...
if($selectedname=="CurrentMonthData" OR $selectedname=="LastMonthData"){
    $deliveredcodq = "SELECT count('Single_Order_Id') FROM `spark_single_order` WHERE `Order_Type`='COD' AND `User_Id`='$user_id' AND `Awb_Number`!='' AND `order_status_show`='Delivered' AND `order_cancel` IS NULL AND `Rec_Time_Date` BETWEEN '$startdatefrom' AND '$enddatefrom'";
}else{
...
// Prepaid Orders
if($selectedname=="CurrentMonthData" OR $selectedname=="LastMonthData"){
    $deliveredpreq = "SELECT count('Single_Order_Id') FROM `spark_single_order` WHERE `Order_Type`='Prepaid' AND `User_Id`='$user_id' AND `Awb_Number`!='' AND `order_status_show`='Delivered' AND `order_cancel` IS NULL AND `Rec_Time_Date` BETWEEN '$startdatefrom' AND '$enddatefrom'";
}else{
...
<div class="">
  ...
  <h5 style="font-size:10px" class=" fontweigh">Prepaid <strong>&ensp;&ensp;-&ensp;&ensp;<?= $deliveredpre ?></strong></h5>
  <h5 style="font-size:10px" class=" fontweigh">COD <strong>&ensp;&ensp;-&ensp;&ensp;<?= $deliveredcod ?></strong></h5>
</div>
