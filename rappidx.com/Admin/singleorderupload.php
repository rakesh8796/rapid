...
<!-- Order Shipping Labels -->
    ...
    // Delhivery 2nd API
    // <!--    Cancel Orders   -->
    echo "<script> window.location.assign('singleorderupload.php?excelfile=Cancel Order')</script>";
    // exit();
...
mysqli_query($conn,$bulkordersq);
    ...
    foreach($rowData as $key => $value){
        ...
        {
            echo "<script> window.location.assign('singleorderupload.php?excelfile=invalid Details')</script>";
        }
    }
    ...
    // Pickup Address
        if(empty($value[17]))
        {
            ...
            if(mysqli_query($conn,$newpickupaddress)){
                ...
                echo "<script> window.location.assign('singleorderupload.php?excelfile=Invalid Data')</script>";
            }
        }else
        {
            $pickupidis = $value[17];
            ...
            if($exists = mysqli_num_rows($alldata))
            {
                ...
                if(mysqli_query($conn,$newpickupaddress)){
                    ...
                    echo "<script> window.location.assign('singleorderupload.php?excelfile=Invalid Data1')</script>";
                }
            }
        }
...
// echo "<br><br>";
            $tdate = date("Y-m-d");
            $singlequery = "INSERT INTO `spark_single_order`(`Order_Type`, `User_Id`,`Name`, `Address`, `State`, `City`, `Mobile`, `Pincode`, `Item_Name`, `Quantity`, `Width`, `Height`, `Length`, `Actual_Weight`, `Total_Amount`, `Invoice_Value`, `Cod_Amount`,`additionaltype`,`Rec_Time_Date`,`uploadtype`,`pickup_id`,`Address_Id`, `pickup_name`,`pickup_mobile`,`pickup_pincode`,`pickup_gstin`,`pickup_address`,`pickup_state`,`pickup_city`,`order_status`,`order_status1`,`order_status_show`) VALUES ('$value[25]','$user_id','$value[0]','$value[1]','$value[12]','$value[13]','$value[2]','$value[3]','$value[5]','$value[6]','$value[7]','$value[8]','$value[9]','$value[15]','$value[16]','$value[10]','$value[11]','$value[14]','$tdate','Single','$value[17]','$value[17]','$value[18]','$value[19]','$value[20]','$value[21]','$value[22]','$value[23]','$value[24]','Pending','Pending','Upload')";
...
// print_r($arrayName);
    ...
    // echo "<br><br>";
            ...
            echo "<script> window.location.assign('singleorderupload.php?excelfile=Update')</script>";
            // exit();

    } catch (Exception $e) {
                echo "<script> window.location.assign('singleorderupload.php?excelfile=No Update')</script>";
    }
