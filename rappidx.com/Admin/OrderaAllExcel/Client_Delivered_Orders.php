...
// error_reporting(1);
$filename=$username."Delivered_Order";
...
{
  $crtawbno = $row['Awb_Number'];
  $deldate = "SELECT * FROM `spark_mis_report` WHERE `awbno`='$crtawbno' ORDER BY `spark_mis_report`.`mis_id` DESC";
  $deldater = mysqli_query($conn,$deldate);
  $deldateres = mysqli_fetch_assoc($deldater);
?>
<tr class="record">
    ...
    <td><?php echo $deldateres['deliverydate']; ?></td>
