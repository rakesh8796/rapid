...
if(isset($loginchekc))
{
    ...
    // $query="SELECT * FROM `asitfa_user` WHERE `User_Email`='$username' AND `User_Password_show`='$password' AND `Active`='1'";
...
if($result['usertype']=="admin"){
    ...
    echo "<script>window.location.assign('Admin/AdminDashboard.php')</script>";
}elseif($result['usertype']=="employee"){
    $_SESSION['paneltitle'] = "Employee Panel";
    header("location:Admin/EmployeeDashboard.php");
    echo "<script>window.location.assign('Admin/EmployeeDashboard.php')</script>";
}elseif($result['usertype']=="retailer"){
    $_SESSION['paneltitle'] = "Retailer Panel";
    
// $cname = $result['Company_Name'];
// $useri = $result['User_Id'];
// $suety = $result['usertype'];
// $uuids = $result['useruinqueidno'];

    // header("location:Admin/RetailerDashboard.php?cname=$cname&useri=$useri&suety=$suety&uuids=$uuids");
    // echo "<script>window.location.assign('Admin/RetailerDashboard.php?cname=$cname&useri=$useri&suety=$suety&uuids=$uuids')</script>";
